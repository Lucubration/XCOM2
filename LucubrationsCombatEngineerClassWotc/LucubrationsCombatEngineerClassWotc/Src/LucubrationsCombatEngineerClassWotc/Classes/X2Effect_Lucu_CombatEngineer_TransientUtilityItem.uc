class X2Effect_Lucu_CombatEngineer_TransientUtilityItem extends X2Effect_Persistent;
		...
		if (UnitState != none)
		{
			...
			if (TransientItemTemplate != none && TransientItemTemplate.InventorySlot == eInvSlot_Utility)
			{
				...
				if (TransientItemState == none)
				{
					...
					`LOG("Lucubration Combat Engineer Class: Transient Item " @ TransientItemState.GetMyTemplateName() @ " state created.");
					...
					if (ItemSetAbilityState != none)
					{
						...
						if (SourceAmmoRef.ObjectID > 0)
							`LOG("Lucubration Combat Engineer Class: Transient Item " @ TransientItemState.GetMyTemplateName() @ " set as source ammo for unit " @ UnitState.GetFullName() @ " ability " @ AbilityTemplateName @ ".");
						else
							`LOG("Lucubration Combat Engineer Class: Transient Item " @ TransientItemState.GetMyTemplateName() @ " set as source weapon for unit " @ UnitState.GetFullName() @ " ability " @ AbilityTemplateName @ ".");
					}
					else
					{
						...
						if (AbilityTemplate != none)
						{
							...
							`LOG("Lucubration Combat Engineer Class: Transient Item " @ TransientItemState.GetMyTemplateName() @ " ability state " @ AbilityTemplateName @ " created and added to unit " @ UnitState.GetFullName() @ ".");
						}
						else
						{
							`LOG("Lucubration Combat Engineer Class: Transient Item " @ TransientItemState.GetMyTemplateName() @ " ability state " @ AbilityTemplateName @ " not created for unit " @ UnitState.GetFullName() @ " (ability template not found).");
						}
					}

					`LOG("Lucubration Combat Engineer Class: Transient Item " @ TransientItemState.GetMyTemplateName() @ " added to unit " @ UnitState.GetFullName() @ " inventory.");
				...
				TransientItemEffectState = XComGameState_Lucu_CombatEngineer_Effect_TransientUtilityItem(NewGameState.CreateStateObject(class'XComGameState_Lucu_CombatEngineer_Effect_TransientUtilityItem'));
				if (CreatedNewItem)
				{
				...
				`LOG("Lucubration Combat Engineer Class: Transient Item passive effect registered for events.");
			}
			else
			{
				`LOG("Lucubration Combat Engineer Class: Transient Item effect inventory item not added (utility weapon template not found).");
			}
		}
		else
		{
			`LOG("Lucubration Combat Engineer Class: Transient Item effect inventory item not added (unit state not found).");
		}
	}
...
static function XComGameState_Lucu_CombatEngineer_Effect_TransientUtilityItem GetEffectComponent(XComGameState_Effect Effect)
{
    if (Effect != none) 
        return XComGameState_Lucu_CombatEngineer_Effect_TransientUtilityItem(Effect.FindComponentObject(class'XComGameState_Lucu_CombatEngineer_Effect_TransientUtilityItem'));
    return none;
}
