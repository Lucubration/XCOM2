class X2TargetingMethod_Lucu_Sniper_SabotRound extends X2TargetingMethod;
...
function Init(AvailableAction InAction)
{
	...
	if (!AbilityTemplate.SkipRenderOfTargetingTemplate)
	{
		...
		if (UnitState.HasSquadsight())
		{
			// With Squadsight, range extends... far
			TileLength = 99.0f;
		}
...
function Update(float DeltaTime)
{
	...
	{
		// Before we call the native stuff, force the ability template's multi-targeting method to update based on our unit's squadsight status.
		// Dirty hack; thank you, Firaxis, for obscurely hiding all this stuff in native functions
		X2AbilityMultiTargetStyle_Lucu_Sniper_SabotRound(Ability.GetMyTemplate().AbilityMultiTargetStyle).UpdateSightRangeLimited(Ability);
