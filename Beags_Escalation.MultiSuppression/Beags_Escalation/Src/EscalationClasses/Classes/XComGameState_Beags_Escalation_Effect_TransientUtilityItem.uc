...
class XComGameState_Beags_Escalation_Effect_TransientUtilityItem extends XComGameState_BaseObject;
...
function EventListenerReturn OnTacticalGameEnd(Object EventData, Object EventSource, XComGameState GameState, Name EventID)
{
    ...
    //`LOG("Beags Escalation: Transient Item 'TacticalGameEnd' event listener delegate invoked.");
		...
		if (ItemState != none)
		{
			...
			if (UnitState != none)
			{
				...
				if (AbilityState != none)
				{
					...
					`LOG("Beags Escalation: Transient Item " @ ItemState.GetMyTemplateName() @ " removed as source weapon for unit " @ UnitState.GetFullName() @ " ability " @ AbilityState.GetMyTemplateName() @ ".");
				}
				else
				{
					`LOG("Beags Escalation: Transient Item " @ ItemState.GetMyTemplateName() @ " not removed as source weapon for unit " @ UnitState.GetFullName() @ " ability " @ AbilityState.GetMyTemplateName() @ " (ability state not found).");
				}
				...
				if (AbilityRef.ObjectID > 0)
				{
					...
					if (AbilityState != none)
					{
						...
						`LOG("Beags Escalation: Transient Item " @ ItemState.GetMyTemplateName() @ " ability " @ AbilityState.GetMyTemplateName() @ " state removed from history.");
					}
					else
					{
						`LOG("Beags Escalation: Transient Item " @ ItemState.GetMyTemplateName() @ " ability " @ AbilityState.GetMyTemplateName() @ " state not removed from history (item state not found).");
					}
				}

				`LOG("Beags Escalation: Transient Item " @ ItemState.GetMyTemplateName() @ " removed from unit " @ UnitState.GetFullName() @ " inventory.");
			}
			else
			{
				`LOG("Beags Escalation: Transient Item " @ ItemState.GetMyTemplateName() @ " not removed from unit inventory (unit state not found).");
			}
	
			// Remove the transient item's gamestate object from history
			NewGameState.RemoveStateObject(ItemRef.ObjectID);

			`LOG("Beags Escalation: Transient " @ ItemState.GetMyTemplateName() @ " state removed from history.");
		}
		else
		{
			`LOG("Beags Escalation: Transient Item not removed from unit inventory (item state not found).");
		}
	}
	...
	`LOG("Beags Escalation: Transient Item passive effect unregistered from events.");
