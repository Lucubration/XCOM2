...
function Vector GetScatterLocation(vector TargetLocation)
{
	...
	local float StandardScatter, StandardRange, MaxScatter, StdDev, ToHit;
	...
	`LOG("Beags Escalation: Rocket ToHit=" @ string(ToHit));
	...
	StdDev = MaxScatter * (100.0f - FMin(ToHit, 100.0f)) / 100.0f;

	`LOG("Beags Escalation: Rocket StdDev=" @ string(StdDev));

	// Scatter based on normal distribution
	ScatterLocation.X = TargetLocation.X + GetScatterLinear(StdDev);
	ScatterLocation.Y = TargetLocation.Y + GetScatterLinear(StdDev);
...
function float GetScatterLinear(float StdDev)
{
	...
	`LOG("Beags Escalation: Rocket Scatter=" @ string(Scatter));
