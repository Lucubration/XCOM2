class X2TargetingMethod_Beags_Escalation_RemoteCone extends X2TargetingMethod;
...
var protected float ConeRadius, ConeAngle;
...
function Init(AvailableAction InAction)
{
	...
	local X2AbilityMultiTarget_Beags_Escalation_Cone ConeMultiTarget;
	...
	ConeMultiTarget = X2AbilityMultiTarget_Beags_Escalation_Cone(AbilityTemplate.AbilityMultiTargetStyle);
	`assert(ConeMultiTarget != none);
	ConeRadius = ConeMultiTarget.GetTargetRadius(Ability);
	ConeAngle = ConeMultiTarget.ConeAngle;
	...
	if (!AbilityTemplate.SkipRenderOfTargetingTemplate)
	{
		...
		ConeActor.InitConeMesh(Cos(ConeAngle / 2)  / class'XComWorldData'.const.WORLD_StepSize, 2 * Sin(ConeAngle / 2) * ConeRadius / class'XComWorldData'.const.WORLD_StepSize);
...
function Update(float DeltaTime)
{
	...
	NewTargetLocation = FiringLocation + (Direction / VSize(Direction)) * ConeRadius;       //  recalibrate based on direction to cursor; must always be as long as possible
...
function GetTargetLocations(out array<Vector> TargetLocations)
{
	...
	TargetLocations.AddItem(FiringLocation); // Store the firing location in the first target location
...
{
	local TTile TestLoc;
	if (TargetLocations.Length == 2 && bGoodTarget)
	{
		if (bRestrictToSquadsightRange)
		{
			// For the remote cone, we care about seeing the initial firing location, not the target location (if indicated, anyways)
