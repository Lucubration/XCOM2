...
var name SuppressionActionPointName;
...
static function array<X2DataTemplate> CreateTemplates()
{
	...
	Templates.AddItem(SuppressionShotMove());
...
{
	local X2AbilityTemplate							Template;
	local X2Effect_Beags_Escalation_FlushDamage		DamageEffect;
	...
	DamageEffect = new class'X2Effect_Beags_Escalation_FlushDamage';
...
static function X2DataTemplate DangerZone()
{
	...
	Template.AdditionalAbilities.AddItem('Beags_Escalation_SuppressionShotMove');
...
static function X2AbilityTemplate DangerZoneSuppression()
{
	...
	local X2Effect_Beags_Escalation_Suppression		SuppressionEffect;
	...
	// Keeping this in here as an easy way to remove normal action points from the shooter
	ReserveActionPointsEffect = new class'X2Effect_ReserveActionPoints';
	//ReserveActionPointsEffect.ReserveType = class'X2Ability_Beags_Escalation_CommonAbilitySet'.default.SuppressionActionPointName;
	...
	SuppressionEffect = new class'X2Effect_Beags_Escalation_Suppression';
	SuppressionEffect.EffectName = 'Suppression';
...
static function X2AbilityTemplate SuppressionShotMove()
{
	local X2AbilityTemplate							Template;
	local X2AbilityTrigger_Event					Trigger;
	local X2Effect_Beags_Escalation_RemoveEffect	RemoveSuppression;

	`CREATE_X2ABILITY_TEMPLATE(Template, 'Beags_Escalation_SuppressionShotMove');
	...
	// Remove one stack of Suppression from the target when this fires
	RemoveSuppression = new class'X2Effect_Beags_Escalation_RemoveEffect';
	RemoveSuppression.EffectNameToRemove = class'X2Effect_Suppression'.default.EffectName;
	RemoveSuppression.bMatchSourceToSource = true;
	RemoveSuppression.bMatchTargetToTarget = true;
	RemoveSuppression.NumberToRemove = 1;
	RemoveSuppression.SetupEffectOnShotContextResult(true, true);
	Template.AddTargetEffect(RemoveSuppression);
	
	Template.OverrideAbilities.AddItem('SuppressionShot');
...
static function X2AbilityTemplate SuppressionShotAttack()
{
	...
	RemoveSuppression.bMatchSourceToSource = true;
	RemoveSuppression.bMatchTargetToTarget = true;
	RemoveSuppression.NumberToRemove = 0;
	RemoveSuppression.SetupEffectOnShotContextResult(true, true);
	Template.AddTargetEffect(RemoveSuppression);
...
static function X2AbilityTemplate SuppressionShotHelper(X2AbilityTemplate Template)
{
	...
	ReserveActionPointCost.AllowedTypes.AddItem(default.SuppressionActionPointName);
	...
	Template.AddTargetEffect(ShotEffect);
	// Damage Effect
	ShotEffect = new class'X2Effect_ApplyWeaponDamage';
	Template.AddTargetEffect(ShotEffect);
...
DefaultProperties
{
	...
	SuppressionActionPointName="Beags_Escalation_Suppression"
}