...
simulated function InitScreen(XComPlayerController InitController, UIMovie InitMovie, optional name InitName)
{
	...
	m_kSlotList.OnSelectionChanged = ListSelectionChanged;
...
simulated function OnInit()
{
	...
	SetTimer(0.2f, false, nameof(SelectFirstListItem)); 	// bsg-jrebar (5/16/17): Update data delay
}

// bsg-jrebar (5/16/17): Select First item and lose/gain focus if using controller
simulated function SelectFirstListItem()
{
	local UIList_SquadEditor SquadList;

	SquadList = UIList_SquadEditor(m_kSlotList);
	// bsg-jrebar (5/30/17): SelectFirstItem now selects for the first item in the list of the first valid inited if one is already selected
	if(SquadList.GetSelectedItem() != none)
	{
		if(!UnitInfos[SquadList.SelectedIndex].bIsInited)
		{
...
simulated function UpdateGamepadFocus()
{
	...
	SquadList = UIList_SquadEditor(m_kSlotList);
	SquadList.OnReceiveFocus(); // bsg-jrebar (5/16/17): squadlist needs focus
}

function ListSelectionChanged(UIList ContainerList, int ItemIndex)
{
	if (`ISCONTROLLERACTIVE && movie.Stack.IsTopScreen(self))
	{
		UpdateNavHelp();
	}
}
...
function CreateSquadInfoItems()
{
	...
	if(m_kSquadLoadout != none)
	{
		...
		if (i > m_NumAllowedUnits)
		{
			`Redscreen("Squad Loadout: '" $ XComGameStateContext_SquadSelect(m_kSquadLoadout.GetContext()).strLoadoutName $ "' exceeds the max number of units. ("$ i $" of "$ m_NumAllowedUnits $" allowed)");
		}
...
simulated function CreatePawns()
{
	...
	{
		UpdatedUnit = XComGameState_Unit(m_kSquadLoadout.ModifyStateObject(class'XComGameState_Unit', Unit.ObjectID));
...
simulated function UpdateData(optional bool bFillSquad)
{
	...
	for(i = 0; i < eMPNumUnitsPerSquad_MAX; ++i)
	{
		...
		if(!bFoundUnit)
		{
			if (!m_bAllowEditing)
			{
				Navigator.RemoveControl(UnitInfos[i]);
				UnitInfos[i].Hide();
			}
...
function OnDisplaySaveSquadBeforeReadyDialog(Name eAction)
{
...
function OnDisplaySaveSquadBeforeExitDialog(Name eAction)
{
	`log(self $ "::" $ GetFuncName() @ `ShowVar(eAction),, 'uixcom_mp');
	if(eAction == 'eUIAction_Accept')
	{
...
simulated function OnReceiveFocus()
{
	...
	SetTimer(0.2f, false, nameof(SelectFirstListItem)); 	// bsg-jrebar (5/30/17): Update data delay
