...
var() bool SkipDamageFlyovers<ToolTip="When damaged, the standard damage flyover will be suppressed">;
...
//Low risk fix for a issues caused by AssociatedTiles being cleared at game state processing time, when it is needed at visualization action time. This list contains the contents of
//AssociatedTiles as of the time it was cleared ( the act of which is used by the game state destruction update ).
var transient array<TTile> PreviousAssociatedTiles; 
...
simulated event PostBeginPlay()
{
	...
	if( DamagedEventActivates.Length == 0 && DestroyedEventActivates.Length == 0 && AnnihilatedEventActivates.Length == 0 )
	{
		...
		if(IsTargetable() && `SecondWaveEnabled('BetaStrike' ) )
		{
			TotalHealth *= class'X2StrategyGameRulesetDataStructures'.default.SecondWaveBetaStrikeHealthMod;
		}
...
static function XComGameState_Destructible SpawnDynamicGameStateDestructible( string DestructibleArchetype, vector SpawnLocation, eTeam Team, optional XComGameState NewGameState )
{
	local XComGameStateHistory History;
	...
	ObjectState = XComGameState_Destructible(NewGameState.CreateNewStateObject(class'XComGameState_Destructible'));

	ObjectState.SpawnedDestructibleArchetype = DestructibleArchetype;
	ObjectState.SpawnedDestructibleLocation = SpawnLocation;
	ObjectState.SpawnedDestructibleTeam = Team;

	ObjectState.TileLocation = `XWORLD.GetTileCoordinatesFromPosition( SpawnLocation );

	DestructibleTemplate = XComDestructibleActor(`CONTENT.RequestGameArchetype(DestructibleArchetype));
	if (DestructibleTemplate != none && DestructibleTemplate.Toughness != none)
	{
		ObjectState.Health = DestructibleTemplate.Toughness.Health;

		if((DestructibleTemplate.Toughness.TargetableBy != TargetableByNone) && `SecondWaveEnabled('BetaStrike' ) )
		{
			ObjectState.Health *= class'X2StrategyGameRulesetDataStructures'.default.SecondWaveBetaStrikeHealthMod;
		}
	}

	ObjectState.bRequiresVisibilityUpdate = true;
...
//Searches the actor's destruction actions for the XComDestructibleActor_Action_RadialDamage event and returns true if it finds one.
native function bool HasRadialDamage();


simulated function GetRadialDamageTiles(out array<TTile> TilesToBeDamaged)
{
	local DestructibleActorEvent kEvent;
	local XComDestructibleActor_Action_RadialDamage RadialEvent;
	local TTile Min, Max, CurrentTile;
	local int X, Y, Z;
	...
	foreach DestroyedEvents(kEvent)
	{
		...
		if( RadialEvent != None )
		{
			RadialEvent.GetBlastExtents(Min, Max);
			for( X = Min.X; X <= Max.X; ++X )
			{
				...
				for( Y = Min.Y; Y <= Max.Y; ++Y )
				{
					...
					{
						CurrentTile.Z = Z;
						TilesToBeDamaged.AddItem(CurrentTile);
					}
				}
			}
		}
...
event OnActionStarted(X2Action StartedAction);
...
simulated state _DestructionStarted extends _DamageState
{
	simulated event BeginState(Name PreviousStateName)
	{
		...
		if (ConsiderForOccupancy( ) && DestructionEmptiesTile( ))
		{
...
simulated state _Destroyed extends _DamageState
{
	simulated event BeginState(Name PreviousStateName)
	{
		bDestroyBegun = false;
		super.BeginState(PreviousStateName);	
		if (!bDoCameraPan || bVisualizerLoad)
			BeginDestroyed();
		if (bVisualizerLoad)
			Tick( TimeBeforeDeath );
	}
