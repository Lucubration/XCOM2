...
exec function MPSetPlayerSync(optional bool bReady=true)
{
	...
	foreach History.IterateByClassType(class'XComGameState_Player', GameStatePlayer)
	{
		...
		//{
			...
			NewPlayerState = XComGameState_Player(NewGameState.ModifyStateObject(class'XComGameState_Player', GameStatePlayer.ObjectID));
...
exec function AddTacticalGameplayTag(Name GameplayTag)
{
	...
	XComHQ = XComGameState_HeadquartersXCom(History.GetSingleGameStateObjectForClass(class'XComGameState_HeadquartersXCom'));
	XComHQ = XComGameState_HeadquartersXCom(NewGameState.ModifyStateObject(class'XComGameState_HeadquartersXCom', XComHQ == None ? -1 : XComHQ.ObjectID));
...
exec function RemoveTacticalGameplayTag(Name GameplayTag)
{
	...
	if( XComHQ.TacticalGameplayTags.Find(GameplayTag) != INDEX_NONE )
	{
		...
		XComHQ = XComGameState_HeadquartersXCom(NewGameState.ModifyStateObject(class'XComGameState_HeadquartersXCom', XComHQ == None ? -1 : XComHQ.ObjectID));
		XComHQ.TacticalGameplayTags.RemoveItem(GameplayTag);
		`GAMERULES.SubmitGameState(NewGameState);
		QuickLaunchUI = UITacticalQuickLaunch(XComPlayerController(Outer).Pres.ScreenStack.GetScreen(class'UITacticalQuickLaunch'));
...
exec function PrintChosenKnowledgeLevels()
{
	local XComGameStateHistory History;
	local XComGameState_AdventChosen ChosenState;
	local string KnowledgeString;

	History = `XCOMHISTORY;
	KnowledgeString = "\n-------------------------------------\nCHOSEN KNOWLEDGE LEVELS:";

	foreach History.IterateByClassType(class'XComGameState_AdventChosen', ChosenState)
	{
		KnowledgeString $= "\n" $ ChosenState.GetChosenClassName();
		KnowledgeString $= "\nKnowledge Level:     " $ ChosenState.KnowledgeGainTitle[ChosenState.GetKnowledgeLevel()];
		KnowledgeString $= "\nRaw Knowledge Score: " $ ChosenState.GetKnowledgeScore(true);
		KnowledgeString $= "\n";
	}
...
exec function X2ChosenListTraits(optional name ChosenType)
{
	if( ChosenType == '' )
	{
		ChosenListTraits('ChosenAssassin');
		ChosenListTraits('ChosenSniper');
		ChosenListTraits('ChosenWarlock');
	}
	else 
	{
		ChosenType = ConvertChosenNameToGroupName(ChosenType);
		ChosenListTraits(ChosenType);
	}
}

function name ConvertChosenNameToGroupName(name ChosenFriendlyName)
{
	if( ChosenFriendlyName == 'Assassin' )
	{
		return 'ChosenAssassin';
	}
	else if( ChosenFriendlyName == 'Hunter' ||
	   ChosenFriendlyName == 'Sniper' ||
	   ChosenFriendlyName == 'ChosenHunter' )
	{
		return 'ChosenSniper';
	}
	else if( ChosenFriendlyName == 'Warlock' )
	{
		return 'ChosenWarlock';
	}
	else
	{
		return ChosenFriendlyName;
	}
}

function ChosenListTraits(name ChosenType)
{
	local XComGameState_AdventChosen ChosenState;
	local name CurrentTrait;

	ChosenState = GetChosenState(ChosenType);

	OutputMsg("     TRAITS for " $ ChosenType);
	OutputMsg("       STRENGTHS:");
	foreach ChosenState.Strengths(CurrentTrait)
	{
		OutputMsg("              " $ CurrentTrait);
	}

	OutputMsg("       WEAKNESSES:");
	foreach ChosenState.Weaknesses(CurrentTrait)
	{
		OutputMsg("              " $ CurrentTrait);
	}
}

exec function X2ChosenAddTrait(name ChosenType, name TraitToAdd)
{
	local XComGameState_AdventChosen ChosenState;
	local XComGameState NewGameState;
	...
	NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("CHEAT: X2ChosenAddTrait");

	ChosenState = GetChosenState(ChosenType);
	ChosenState = XComGameState_AdventChosen(NewGameState.ModifyStateObject(class'XComGameState_AdventChosen', ChosenState.ObjectID));

	ChosenState.AddTrait(TraitToAdd);
...
exec function X2ChosenRemoveTrait(name ChosenType, name TraitToRemove)
{
	local XComGameState_AdventChosen ChosenState;
	local XComGameState NewGameState;
	...
	NewGameState = class'XComGameStateContext_ChangeContainer'.static.CreateChangeState("CHEAT: X2ChosenRemoveTrait");

	ChosenState = GetChosenState(ChosenType);
	ChosenState = XComGameState_AdventChosen(NewGameState.ModifyStateObject(class'XComGameState_AdventChosen', ChosenState.ObjectID));

	ChosenState.RemoveTrait(TraitToRemove);
...
function XComGameState_AdventChosen GetChosenState(name ChosenType)
{
	local XComGameStateHistory History;
	...
	if( AlienHQ != None )
	{
		ChosenState = AlienHQ.GetChosenOfTemplate(ChosenType);
	}
