...
static function array<XComInteractPoint> GetUnitInteractionPoints(XComGameState_Unit Unit, UnitInterationType Type)
{
	...
	local array<TTile> UnitLocations;
	local int Index, i;

	World = class'XComWorldData'.static.GetWorldData();

	// Change this from UnitLocation to UnitLocations and loop over them
	if (Unit.UnitSize > 1)
	{
		Unit.GetVisibilityForLocation(Unit.TileLocation, UnitLocations);
	}
	else
	{
		UnitLocations.AddItem(Unit.TileLocation);
	}
