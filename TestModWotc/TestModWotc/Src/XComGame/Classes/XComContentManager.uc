...
enum EColorPalette
{
	...
	ePalette_FontColors<DisplayName = Font Colors>,
};
...
struct native MapContentReductionMap
{
	var string DefaultPackageName;
	var string ReducedPackageName;
};
...
var config protected array<string> AllowTickLOD_FXPackages; //A list of packages for which the particle effects should use the TickLOD system.
var config protected array<MapContentReductionMap> ReducedPackageList; //Used to swap in lower detail particle effects - used by EffectCues. Could be used for other resources potentially.
var private native Set_Mirror AllowTickLOD_FXPackagesSet{ TSet<FName> }; //Stores AllowTickLOD_FXPackages in a set at startup time for quicker lookup when it is needed following each map load

var transient init array<string> RequiredArchetypes; // A list of archetypes to load when CacheContent is called
...
var protectedwrite transient array<XComPerkContent> PerkContent;							// mostly for easy GC references
var private native Map_Mirror PerkPackageMap{ TMap<FName, TArray<FString> > };				// Map of ability names to package names with associated perks in them
var private native Map_Mirror PerkContentMap{ TMap<FName, TArray<UXComPerkContent*> > };	// Map of ability names to the currently loaded perks.
...
// The Path name to the effect that should be played when a unit warps in via Chosen Teleport
var config string ChosenTeleportInEffectPathName;

// The Path name to the effect that should be played when a chosen reinforcements unit warps in
var config string ChosenReinforcementsEffectPathName;

// The Path name to the effect that should be played to represent the PsiGate
var config string ReinforcementDropdownWarningEffectPathName;

// The Path name to the effect that should be played when a unit warps in via psi gate
var config string ReinforcementDropdownEffectPathName;
...
native final function AppendAbilityPerks(name AbilityName, XComUnitPawnNativeBase UnitPawn);
native final function  CachePerkContent(name AbilityName);
native final function  ClearPerkContentCache();
native final function  BuildPerkPackageCache();
...
cpptext
{
	...
	UBOOL GetReducedPackagePath(FString& InOutPackagePath) const; //Will replace the package name in InOutPackagePath with one from ReducedPackageList if there is a match
	...
	INT CacheContentAsync(TArray<FString>& ArchetypesToLoad, UObject* CallbackObject, FScriptDelegate CallbackFn=FScriptDelegate(EC_EventParm));
