...
function PerformChallengeModeGetTopScores()
{
	...
	ChallengeModeInterface.PerformChallengeModeGetTopGameScores(m_arrIntervals[Index].IntervalName);
}
...
function OnReceivedChallengeModeIntervalEntry(qword IntervalSeedID, int ExpirationDate, int TimeLength, EChallengeStateType IntervalState, string IntervalName, array<byte> StartState)
{
	local int Idx;
...
function SetupMapData()
{
	...
	ChallengeMission = XComGameState_MissionSite(m_NewStartState.CreateNewStateObject(class'XComGameState_MissionSite'));
	...
	for( RewardIndex = 0; RewardIndex < MissionState.Rewards.Length; ++RewardIndex )
	{
		...
		if( RewardStateObject.IsPersonnelType() )
		{
			...
			SendSoldierState = XComGameState_Unit( NewStartState.ModifyStateObject(class'XComGameState_Unit', RewardStateObject.RewardObjectReference.ObjectID) );
