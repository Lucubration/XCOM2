...
simulated function WhitelistRetrieved(bool bWasSuccessful, EOnlineEventType EventType)
{
	...
	if (EventType == EOET_Auth)
	{
		...
		if (bWasSuccessful)
		{
			...
			// XComPlayerController(class'Engine'.static.GetCurrentWorldInfo().GRI.GetALocalPlayerController()).Pres.QueueAnchoredMessage("Copy protection passed ", 0.55f, 0.8f, BOTTOM_CENTER, 5.0f,, eIcon_GenericCircle);
		}
		else
		{
			`log("Copy protection failed. Contact the developer and include this information: ");
			XComPlayerController(class'Engine'.static.GetCurrentWorldInfo().GRI.GetALocalPlayerController()).Pres.QueueAnchoredMessage("Blast processing issue. Contact the developer and include this information.", 0.55f, 0.8f, BOTTOM_CENTER, 20.0f,, eIcon_ExclamationMark);
