...
var protectedwrite bool bCharacterExclusive; // if the filter should only select body parts specifically designated for this character
...
function AddCharacterFilter(Name InCharacterName, optional bool bExclusive)
{
	CharacterName = InCharacterName;
	bCharacterExclusive = bExclusive;
}
...
{
	if (Template.CharacterTemplate == CharacterName)
	{
		return true;
	}
	else if (!bCharacterExclusive)
	{
		return Template.CharacterTemplate == '';
	}
	else
	{
		return false;
...
function bool FilterByCharacterExclusive(X2BodyPartTemplate Template)
{
	return (Template.CharacterTemplate == CharacterName);
}
...
function bool FilterByClass(X2BodyPartTemplate Template)
{
	...
	//Automatically pass if no soldier class is specific
	if (Template.ReqClass != '')
	{
		//See if it has been researched
		History = `XCOMHISTORY;
		XComHQ = XComGameState_HeadquartersXCom(History.GetSingleGameStateObjectForClass(class'XComGameState_HeadquartersXCom', true));
		if (XComHQ != none)
		{
			return XComHQ.MeetsSoldierClassGates(Template.ReqClass);
		}		
	}
...
function bool FilterByGenderAndRaceAndCharacterAndClass(X2BodyPartTemplate Template)
{
	return FilterByGenderAndRaceAndCharacter(Template) && FilterByClass(Template);
}
...
function bool FilterByGenderAndCharacterExclusive(X2BodyPartTemplate Template)
{
	return FilterByGender(Template) && FilterByCharacterExclusive(Template);
}
...
function bool FilterByGenderAndNonSpecializedAndClassAndTechAndArmor(X2BodyPartTemplate Template)
{
	return FilterByGender(Template) && FilterByNonSpecialized(Template) && FilterByClass(Template) && FilterByTech(Template) && FilterByArmor(Template);
}
...
function bool FilterByTorsoAndArmorMatch(X2BodyPartTemplate Template)
{
	...
	bMatched = FilterByGenderAndNonSpecialized(Template) &&
		...
		(TorsoTemplate.ArmorTemplate == Template.ArmorTemplate || Template.bAnyArmor) &&
