...
{	
	...
	var int Bias; //Values greater than 1 will increase the likelyhood of the map appearing
};
...
var config privatewrite bool bBlockingLoadOverride;
var config privatewrite bool bSkipLoadingConfirmation;
var config privatewrite bool bPrecacheMaps;
var config privatewrite bool bDelayLoadBlueprints;
...
var float LoadStartedTime;
var bool bLoadTimingStarted; //This allows multiple / redundant calls to the start of timing. Due to the distributed nature of entering missions, the time at which
							 //LoadTimer_Start should be called may vary. bLoadTimingStarted ensures that the start time is set at the first call.
...
native function LoadTimer_Start();
native function LoadTimer_Stop();

native function PrecacheMapsForPlotType(const string PlotType);
