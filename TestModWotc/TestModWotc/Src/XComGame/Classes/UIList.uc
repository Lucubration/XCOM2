...
var public bool					        bStickyHighlight; //Mouse-over refresh style sticky list, most common 
var public bool					        bStickyClickyHighlight; //Click to stick highlight
...
var bool				bSortStandardDepths; //Will refresh z-sort on the flash side after all items are loaded. 
var public int                          LeftMaskOffset;      // Expands the masks left border and widens it by that same amount
var public int							ItemWidthBuffer;     // Buffer space to show items at the end of a long list so they fit within the mask
...
{
	local UIPanel Panel; 
	Panel = Spawn(ItemClass, ItemContainer);
	
	if( bSortStandardDepths ) 
...
simulated function ItemInited(UIPanel Panel)
{
	...
	//Once all items are loade4d, then call for sorting. 
	if( bSortStandardDepths )
	{
		for( i = ItemCount-1; i > -1; i-- )
		{
			GetItem(i).MoveToHighestDepth();
		}
	}
}
...
{
	if (ItemContainer != none && Index >= 0 && Index < ItemContainer.ChildPanels.Length)
...
{
	if (ItemContainer != none && SelectedIndex > INDEX_NONE && SelectedIndex < ItemContainer.ChildPanels.Length)
	{
...
simulated function OnChildMouseEvent(UIPanel Control, int cmd)
{
	...
	case class'UIUtilities_Input'.const.FXS_L_MOUSE_DRAG_OVER:
		if( HasItem(Control) && !bStickyClickyHighlight )
		{
			SetSelectedIndex(GetItemIndex(Control));
		}
		break;
...
simulated function RealizeItems(optional int StartingIndex)
{
	...
	TotalItemSize = NextItemPosition+ItemWidthBuffer;
}
...
simulated function RealizeMaskAndScrollbar()
{
	...
	if(RelevantSize != 0 && TotalItemSize > RelevantSize)
	{
		...
		Mask.SetSize(Width-LeftMaskOffset, Height);
		Mask.SetX(LeftMaskOffset);
