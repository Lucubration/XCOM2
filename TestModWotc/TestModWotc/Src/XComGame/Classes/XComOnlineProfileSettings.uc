...
function PostLoadData()
{
	...
	Movies.AddItem(Pres.Get2DMovie());
	Movies.AddItem(Pres.Get3DMovie());
	Movies.AddItem(Pres.GetModalMovie());
...
function ExtendedLaunch_InitToDefaults()
{
	...
	History.AddGameStateToHistory( TacticalGameStartState );

	class'XComGameStateContext_StrategyGameRule'.static.CreateStrategyGameStart( 
		TacticalGameStartState, 
		, 
		,
		,
		,
	...
	//***
	foreach TacticalGameStartState.IterateByClassType(class'XComGameState_Unit', ExamineUnit)
	{
	...
	{
		TacticalGameStartState.PurgeGameStateForObjectID(RemoveUnits[Index]);
	}
	//***
...
{	
	...
	AddPlayerState = class'XComGameState_Player'.static.CreatePlayer(StartState, eTeam_TheLost);
	BattleDataState.PlayerTurnOrder.AddItem(AddPlayerState.GetReference());

	AddPlayerState = class'XComGameState_Player'.static.CreatePlayer(StartState, eTeam_Resistance);
	BattleDataState.PlayerTurnOrder.AddItem(AddPlayerState.GetReference());
...
function HACK_RemoveInstanceDataFromGameState(out XComGameState StartState)
{
	...
	{
		StateObject.EndTacticalPlay(StartState);
	}
}
...
event ReadTacticalGameStartState(XComGameState OutStartState)
{
	...
	if( OutStartState != none )
	{
		...
		FIXUP_MaybeAddResistanceTeam( OutStartState );
	}
}

function FIXUP_MaybeAddResistanceTeam( XComGameState OutStartState )
{
	local XComGameState_BattleData BattleDataState;
	local XComGameState_Player PlayerState;

	foreach OutStartState.IterateByClassType( class'XComGameState_Player', PlayerState )
	{
		if (PlayerState.GetTeam() == eTeam_Resistance)
			break;
	}

	if (PlayerState == none)
	{
		foreach OutStartState.IterateByClassType( class'XComGameState_BattleData', BattleDataState )
		{
			break;
		}

		PlayerState = class'XComGameState_Player'.static.CreatePlayer(OutStartState, eTeam_Resistance);
		BattleDataState.PlayerTurnOrder.AddItem(PlayerState.GetReference());
	}
}
...
simulated function XComGameState CreateSquadSelectState(int iLoadoutId, string strNewLoadoutName, string strLanguage)
{
	...
	kGameStatePlayer = XComGameState_Player(kSquadState.CreateNewStateObject(class'XComGameState_Player'));
