...
{
	local ShuttleToScreenType ShuttleScreenType;
	ShuttleScreenType = `ONLINEEVENTMGR.GetShuttleToScreen();
	`log(`location @ `ShowVar(IsA('X2MPLobbyPresentationLayer')) @ `ShowEnum(ShuttleToScreenType, ShuttleScreenType, ShuttleScreenType), true, 'XCom_Online');
	...
	if( !IsA('X2MPLobbyPresentationLayer') )
	{
		...
		{
			switch (ShuttleScreenType)
			{
			case EOEMSTST_MPMainMenu:
				ShuttleToMPMainMenu();
				break;

			case EOEMSTST_MPInviteLoadout:
				ShuttleToMPInviteLoadoutMenu();
				break;

			case EOEMSTST_ChallengeScreen:
	...
	{
		if( `ONLINEEVENTMGR.GetShuttleToMPInviteLoadout() )
		{
			ShuttleToMPInviteLoadoutMenu();
		}
	}
...
//`endif
	...
	`XENGINE.m_kPhotoManager.FillPropagandaTextureArray(ePWM_Recent, -1);
...
simulated private function ShuttleToChallengeMenu()
{
...
simulated private function ShuttleToMPMainMenu()
{
	...
	if (`ONLINEEVENTMGR.GetShuttleToMPMainMenu())
	{
		`ONLINEEVENTMGR.ClearShuttleToScreen();
	}
}
...
function UIActionCallback_OnlineNoNetworkConnectionDialogCallback( Name eAction ) {
	// Go into MP state anyways, options will be grayed out if player is not connected
	StartMPShellState();
}
function UIActionCallback_OnlineLoginFailedDialogCallback( Name eAction ) {
	// Go into MP state anyways, options will be grayed out if player is not connected
	StartMPShellState();
}
function UIActionCallback_OnlinePlayPermissionFailedDialogCallback( Name eAction ) {
	// Go into MP state anyways, options will be grayed out if player is not connected
	StartMPShellState();
}
function UIActionCallback_OnlineChatPermissionFailedDialogCallback( Name eAction ) {
	// Go into MP state anyways, options will be grayed out if player is not connected
	StartMPShellState();
}
...
function UIActionCallback_ProcessSystemErrorMessages( Name eAction ) 
{
...
function OSSAutomatchRanked()
{
	...
	local bool bAllRankedGameDataValid;

	`log(`location, true, 'XCom_Online');
	bAllRankedGameDataValid = class'Helpers'.static.NetAllRankedGameDataValid();
	if(!bAllRankedGameDataValid)
	{
		`RedScreen("Modified game data detected, ranked play should be disabled. @ttalley");
	}
...
{
	XComHeadquartersCamera(GetCamera()).StartRoomViewNamed(name(strLocation), fInterpTime , bSkipBaseViewTransition, ForceLocation, ForceRotation);
}
