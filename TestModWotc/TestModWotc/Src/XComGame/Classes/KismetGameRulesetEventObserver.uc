...
var privatewrite XComGameState PostBuildTriggerState;
...
{	
	...
	EventManager.RegisterForEvent(ThisObj, 'PlayerTurnEnded', CheckForTeamHavingNoPlayableUnits_TurnEnded, ELD_OnStateSubmitted, UNIT_REMOVED_EVENTPRIORITY);
}
...
private function EventListenerReturn CheckForBleedingOutUnits(Object EventData, Object EventSource, XComGameState NewGameState, Name InEventID, Object CallbackData)
{
	...
	CheckForTeamHavingNoPlayableUnits(EventData, EventSource, NewGameState, '', CallbackData);
...
private function CheckPlayableUnits_Internal( XComGameState_Player PlayerObject, XComGameState GameState )
{
	...
	if( TriggeredNoPlayableUnits_PlayerList.Find(PlayerVisualizer) == INDEX_NONE ) //See if we have already triggered for this player
	{
...
{	
	...
	{
		CheckPlayableUnits_Internal( PlayerObject, NewGameState );
	}

	return ELR_NoInterrupt;
}

private function EventListenerReturn CheckForTeamHavingNoPlayableUnits_TurnEnded(Object EventData, Object EventSource, XComGameState NewGameState, Name InEventID, Object CallbackData)
{
	local XComGameState_Player PlayerObject;

	if(NoPlayableUnitsRemainingEvents.Length == 0) return ELR_NoInterrupt; // nothing to do

	PlayerObject = XComGameState_Player( EventData );

	CheckPlayableUnits_Internal( PlayerObject, none );
