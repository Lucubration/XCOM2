...
//  PURPOSE: 3/4 "game view" camera that keeps all points of interest within the screen's safe zone.
//           
//---------------------------------------------------------------------------------------
...
// the maximum distance away from the centerpoint this camera is allowed to pull back
var private const config float MaximumMidpointZoomOut;
...
// An actor that is currently being targeted
...
// if true, this midpoint camera will continuously update to follow all of the actors it is framing
...
function Activated(TPOV CurrentPOV, X2Camera PreviousActiveCamera, X2Camera_LookAt LastActiveLookAtCamera)
{
	...
	RecomputeLookatPointAndZoom(false);
...
function AddFocusPoint(Vector FocusPoint, optional bool bAllowDuplicates = false)
{
...
function ClearFocusActors()
{
	FocusActors.Remove(0, FocusActors.Length);
}
...
{
	if( bFollowMovingActors )
	{
		RecomputeLookatPointAndZoom(true);
	}
	return CachedLookAt;
}
...
private function GetFocusPointsInternal(out array<vector> OutFocusPoints, bool OnlyFocusActors)
{
	...
	local XComDestructibleActor FocusDestructible;
	...
	foreach FocusActors(FocusActor)
	{
		...
		FocusDestructible = XComDestructibleActor(FocusActor);

		if(FocusUnit != none)
		{
			if( !OnlyFocusActors || FocusUnit.IsVisible() )
			{
				// special case actors to include their head and their feet

				// only include the head if we're not following the actors, otherwise head bobbing animations will negatively impact the framing camera's motion
				if( !OnlyFocusActors )
				{
		...
		else if (FocusDestructible != none)
		{
			FocusPoint = FocusDestructible.GetTargetingFocusLocation( );
			ValidateFocusPoint( FocusPoint );
			OutFocusPoints.AddItem(FocusPoint);
		}
		else
	...
	if( OutFocusPoints.Length == 0 && FocusActors.Length > 0 )
	{
		FocusPoint = FocusActors[0].Location;
		ValidateFocusPoint(FocusPoint);
		OutFocusPoints.AddItem(FocusPoint);
	}
...
private function float GetZOfHighestFocusPoint(const out array<Vector> AllFocusPoints)
{
	local Vector FocusPoint;
	local float HighestZ;

	foreach AllFocusPoints(FocusPoint)
	{
		HighestZ = FMax(HighestZ, FocusPoint.Z);
	}
...
/// </summary>
function RecomputeLookatPointAndZoom(bool OnlyFollowActors)
{
	...
	GetFocusPointsInternal(AllFocusPoints, OnlyFollowActors);
	foreach AllFocusPoints(FocusPoint)
	{
...
// 	`Battle.DrawDebugSphere(FrustumIntersect, 20, 20, 0, 255, 0);
	...
	if( OnlyFollowActors )
	{
		CachedLookAt.Z = GetZOfHighestFocusPoint(AllFocusPoints);
	}
	else
	{
...
/// planes of the given frustum. This should be replaced with the native GetFrustumPlanes() function at some point.
/// </summary>
...
/// </summary>
function Vector GetTargetLocation(optional bool bUsePredictedTargetHeadLocation = true)
{
