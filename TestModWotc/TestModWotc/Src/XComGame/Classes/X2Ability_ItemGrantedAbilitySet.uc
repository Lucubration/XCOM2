...
static function array<X2DataTemplate> CreateTemplates()
{
	...
	// XPACK
	Templates.AddItem(RefractionFieldAbility());
	Templates.AddItem(SustainingSphereAbility());
	Templates.AddItem(SustainingSphereTriggerAbility());
...
//BEGIN AUTOGENERATED CODE: Template Overrides 'ExplosiveDeviceDetonate'
	Template.bFrameEvenWhenUnitIsHidden = true;
//END AUTOGENERATED CODE: Template Overrides 'ExplosiveDeviceDetonate'
...
static function X2AbilityTemplate SKULLJACKAbility()
{
	...
	Template.BuildInterruptGameStateFn = SKULLJACK_BuildInterruptGameState;
	Template.BuildVisualizationFn = SKULLJACK_BuildVisualization;
	//BEGIN AUTOGENERATED CODE: Template Overrides 'SKULLJACKAbility'	
	Template.bFrameEvenWhenUnitIsHidden = true;
	//END AUTOGENERATED CODE: Template Overrides 'SKULLJACKAbility'
...
simulated static function XComGameState SKULLJACK_BuildInterruptGameState(XComGameStateContext Context, int InterruptStep, EInterruptionStatus InterruptionStatus)
{
	...
	if (NewGameState == none) // don't do work on the non-interruption end-state
		return NewGameState;
...
simulated function SKULLJACK_BuildVisualization(XComGameState VisualizeGameState)
{
	local XComGameStateHistory History;
	...
	if( Context.IsResultContextHit() )
	{
		...
		class'X2Action_SKULLJACK'.static.AddToVisualizationTree(ActionMetadata, Context, false, ActionMetadata.LastActionAdded);
	}
	else
	{
...
static function ScorchCircuits_VisualizationMerge(XComGameStateContext_Ability Context, int OuterIndex, int InnerIndex)
{
	...
	if (XGUnit(`XCOMHISTORY.GetVisualizer(Context.InputContext.PrimaryTarget.ObjectID)) == VisualizationActionMetadatas[OuterIndex].VisualizeActor)
	{
		...
		{
			EnterCoverAction = X2Action_EnterCover(VisualizationActionMetadatas[OuterIndex].TrackActions[TrackIndex]);
	...
	else
	{
		...
		for (TrackIndex = InnerIndex - 1; TrackIndex >= 0; --TrackIndex)
		{
			DamageAction = X2Action_ApplyWeaponDamageToUnit(VisualizationActionMetadatas[OuterIndex].TrackActions[TrackIndex]);
...
static function X2AbilityTemplate RefractionFieldAbility()
{
	local X2AbilityTemplate             Template;

	Template = class'X2Ability_RangerAbilitySet'.static.Stealth('RefractionFieldAbility');

	Template.AbilityCosts.AddItem(new class'X2AbilityCost_ConsumeItem');
//BEGIN AUTOGENERATED CODE: Template Overrides 'RefractionFieldAbility'
	Template.IconImage = "img:///UILibrary_XPACK_Common.PerkIcons.UIPerk_refractionfield";
//END AUTOGENERATED CODE: Template Overrides 'RefractionFieldAbility'

	return Template;
}

static function X2AbilityTemplate SustainingSphereAbility()
{
	local X2AbilityTemplate             Template;
	local X2Effect_SustainingSphere     SustainEffect;

	`CREATE_X2ABILITY_TEMPLATE(Template, 'SustainingSphereAbility');
	...
	Template.AdditionalAbilities.AddItem('SustainingSphereTriggeredAbility');

	Template.AbilityToHitCalc = default.DeadEye;
	Template.AbilityTargetStyle = default.SelfTarget;
	Template.AbilityTriggers.AddItem(default.UnitPostBeginPlayTrigger);

	SustainEffect = new class'X2Effect_SustainingSphere';
...
static function X2AbilityTemplate SustainingSphereTriggerAbility()
{
	local X2AbilityTemplate                 Template;
	...
	`CREATE_X2ABILITY_TEMPLATE(Template, 'SustainingSphereTriggeredAbility');
	...
	EventTrigger.ListenerData.Priority = 25;	// Needs to trigger after SustainTriggered
	Template.AbilityTriggers.AddItem(EventTrigger);

	EffectsCondition = new class'X2Condition_UnitEffects';
	EffectsCondition.AddExcludeEffect(class'X2Effect_Stasis'.default.EffectName, 'AA_DuplicateEffectIgnored');
	Template.AbilityShooterConditions.AddItem(EffectsCondition);
	...
	Template.PostActivationEvents.AddItem(class'X2Effect_SustainingSphere'.default.SustainTriggeredEvent);
