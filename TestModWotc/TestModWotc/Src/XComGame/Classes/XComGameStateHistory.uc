...
var const config bool EnableSingletonStateOptimization;
var const config bool EnableThreadedStateSubmission;
...
// When doing a latent history update on a thread, the main game thread (and possible others) will still need to query the history
// at the current non-latent head. To keep that snappy, we copy the object cache accelerators structures point the latent update
// started, and return all non-latent queries from the copies.
var private{private} XComGameState LatentReadStateObjectCache;
var private{private} native Map_Mirror LatentReadStateObjectCacheTypeMap{ TMap<FName, TArray<UXComGameState_BaseObject*> > };
...
// When building a game state on another thread, the ruleset will set this value so that threads that are not doing the latent
// update (and hence should be read only) will only access the history up to the point of the last completed history index.
var native private {private} const ThreadSafeCounter	LatentUpdateThreadID{ mutable FThreadSafeCounter };
var native private {private} const ThreadSafeCounter	LatentUpdateReadIndex{ mutable FThreadSafeCounter };
...
var private{private} int NumArchivedFrames;			//The number of indices that are no longer in the history array because of archiving
...
public: 
	...
	void ReadFromByteArray(TArray<BYTE>& InData, TArray<UObject*>& AdditionalObjects, UBOOL bResetHistory = TRUE, INT PackageFileVersion = GPackageFileVersion, INT PackageFileLicenseeVersion = GPackageFileLicenseeVersion); //AdditionalObjects specifies additional objects to serialize with the history
	...
	// Called from the ruleset when starting and stopping a latent update that needs to appear atomic to other threads.
	void StartLatentUpdate();
	void SetLatentUpdateThreadID(int ThreadID);
	UBOOL IsReadOnlyThread() const;
	...
	/// <summary>
	/// Called during the unreal transition to a new map url. Applies the pending start state
	/// for the new map on the level transition boundary, avoiding and tick timing issues by
	// pulling the history out from under the world before it's fully ready to go
	/// </summary>
	void ApplyPendingStartState();

	void CrashDump(FCrashDumpArchive &Ar);

private:
	bool ShouldValidateContext(const UXComGameStateContext_Ability* Context);

	// The publicly exposed version of AddGameStateToHistory does some special processing for start states.
	// This native version is the "real" function, and always adds the specified state directly to the history.
	void AddGameStateToHistoryInternal(UXComGameState* GameState);
}

// Called by the ruleset upon completion of a latent, threaded game state submission.
// Notifies the history that it is available for updating from the main thread again.
native function EndLatentUpdate();
...
/// </summary>
native final function ArchiveHistory(string ArchiveDescriptor, optional bool SkipTacticalTransientTypes = false);
...
/// <summary>
/// If a 'start state' is the top element on the history, purges a state from it.  Optionally rebuilds the state object cache to reflect the
/// removal.  When being called for a large number of objects, the state cache rebuild can be prevented and the caller is responsible for
/// calling UpdateStateObjectCache when all desired states have been purged.
/// </summary>
/// <param name="ObjectID">Specifies which state object to purge</param>
/// <param name="RebuildCache">Specifies whether or not to update the state object cache.  When false, UpdateStateObjectCache should be called
///							directly after all desired states have been purged.</param>
native final function PurgeObjectIDFromStartState(int ObjectID, bool RebuildCache = TRUE);

native final function UpdateStateObjectCache(const optional XComGameState NewState, optional int HistoryIndex = -1);
...
/// <param name="BackwardIterationLimit">How far backwards in the history iteration into the past should go. Defaults to 0 (the start of time)
native final iterator function IterateContextsByClassType( class<object> SearchClass, out object ContextObject, optional EGameStateReturnType DesiredReturnType=eReturnType_Reference, optional bool IterateIntoThePast = true, int BackwardIterationLimit = 0 );
...
native function AddThreadHistoryLock();
native function RemoveThreadHistoryLock();
