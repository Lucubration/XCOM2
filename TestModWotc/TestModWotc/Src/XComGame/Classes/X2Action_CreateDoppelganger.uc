...
function Init()
{
	...
	History = `XCOMHISTORY;
	DoppelgangerUnit = XGUnit(Metadata.VisualizeActor);
...
simulated state Executing
{
	...
	function CopyAppearance(XComUnitPawn SourcePawn, XComUnitPawn DestinationPawn)
	{
		...
		else
		{
			...
			CopyMeshAndMaterials(SourcePawn.m_kLeftForearmMC, DestinationPawn.m_kLeftForearmMC, SourcePawn.Mesh, DestinationPawn.Mesh, SourcePawn.LightEnvironment);
			CopyMeshAndMaterials(SourcePawn.m_kRightForearmMC, DestinationPawn.m_kRightForearmMC, SourcePawn.Mesh, DestinationPawn.Mesh, SourcePawn.LightEnvironment);
			...
			CopyMeshAndMaterials(SourcePawn.m_kThighsMC, DestinationPawn.m_kThighsMC, SourcePawn.Mesh, DestinationPawn.Mesh, SourcePawn.LightEnvironment);
			CopyMeshAndMaterials(SourcePawn.m_kShinsMC, DestinationPawn.m_kShinsMC, SourcePawn.Mesh, DestinationPawn.Mesh, SourcePawn.LightEnvironment);
			CopyMeshAndMaterials(SourcePawn.m_kTorsoDecoMC, DestinationPawn.m_kTorsoDecoMC, SourcePawn.Mesh, DestinationPawn.Mesh, SourcePawn.LightEnvironment);
	...
	function ChangePerkTarget()
	{
		...
		local XComGameState_Effect ReplacementEffect;
		...
		if( ReanimatorAbilityState != none )
		{
			...
			{
				foreach StateChangeContext.AssociatedState.IterateByClassType( class'XComGameState_Effect', ReplacementEffect )
				{
					if (DoppelgangerUnitState.AffectedByEffects.Find( 'ObjectID', ReplacementEffect.ObjectID ) != INDEX_NONE)
						break;
				}
			}

			class'XComPerkContent'.static.GetAssociatedPerkInstances(Perks, CasterPawn, ReanimatorAbilityState.GetMyTemplateName(), StateChangeContext.AssociatedState.HistoryIndex);
			for( i = 0; i < Perks.Length; ++i )
			{
				if (DoppelgangerUnitState.IsDead())
					Perks[i].RemovePerkTarget(OriginalUnit);
				else
					Perks[i].ReplacePerkTarget(OriginalUnit, DoppelgangerUnit, ReplacementEffect);
			}
		}
	}
