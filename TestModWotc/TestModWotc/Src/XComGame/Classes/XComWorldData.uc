...
var transient bool bTickIsDisabled;
...
struct native TileData
{
	...
	var init array<Actor>	TileOccupiers;
...
struct native DeferredRegisterActor
{
	var Actor Actor;
	var INT SightRadius; 
	...
	structcpptext
	{
		FDeferredRegisterActor(AActor *InActor, INT InSightRadius, FVector &InPositionOffset) :
		Actor(InActor),
...
var transient protected init array<DeferredRegisterActor> DeferredRegisterViewers;
...
var transient private init array<XComBuildingVolume> BuildingVolumeCache;
...
cpptext
{
	...
	{	
		return !HasAnyFlags( RF_Unreachable | RF_AsyncLoading ) && !bTickIsDisabled;
	}
	...
	void ComputeVisibilityVoxels(const FTTile &Tile, UBOOL Refresh = FALSE, INT HistoryIndex = -1);
	INT  ComputeTileSupportsLargeUnits(const FTTile &Tile);
	void ComputeBlockingFlags(const FTTile& Tile, FTileData& TileData, UBOOL bIncludeRenderVisibility = FALSE, INT HistoryIndex = -1 );
	void ComputeTileNeighbors(const struct FTileDesc& Tile, BYTE DirectionsToCompute = 0xFF, INT HistoryIndex = -1);
	void ComputeLevelBorderTiles(const struct FTTile& Tile);
	void MaybeModifyTraversalForWindow(FPathingTraversalData &TraversalData, const struct FTTile& SourceTile, const struct FTTile& NeighborTile );
	...
	void FindAndAddDoorCrossing(const FTTile& SourceTile, const FTTile& NeighborTile, INT HistoryIndex = -1);
	void ComputeInteractions(const FTTile& Tile, INT HistoryIndex = -1); //Might affect regular neighbor nodes ( eg. doors ), so has to happen prior to that processing
...
native function BeginFOWViewerUpdates( Actor Viewer );
native function EndFOWViewerUpdates( Actor Viewer );
native function ForceFOWViewerUpdate( Actor Viewer );
native function ForceUpdateAllFOWViewers( );
...
native function RuntimeRegisterDestructibleActor( XComDestructibleActor Destructible );
native function AddDestructibleToBuildingVis( XComDestructibleActor Destructible );
native function RemoveDestructibleFromBuildingVis( XComDestructibleActor Destructible );
native function RemoveFromBuildingVolumeCache( XComBuildingVolume BuildingVolume );
...
native function bool IsFloorTileAndValidDestination(const out TTile Tile, optional XComGameState_Unit UnitState );
native function bool TileHasFloorThatIsDestructible(const out TTile kTile);
native function bool IsFloorTile( const out TTile kTile, optional Actor IgnoreFloor ) const;
native function Actor GetFloorTileActor(const out TTile kTile) const;
...
native function bool IsAdjacentTileBlocked(TTile SourceTile, const out TTile DestinationTile) const;
...
//------------------------------------------------------------------------------
native function AddActorTileData( Actor NewActor, optional XComGameState UpdateState );
native function RemoveActorTileData( Actor ExistingActor, bool Immediate = false, optional XComGameState UpdateState );
native function RefreshActorTileData( Actor ExistingActor, optional XComGameState UpdateState );
native function UpdateTileDataCache( XComGameState UpdateState, optional XComGameState_EnvironmentDamage DamageStateObject, bool Immediate = false );
...
native function array<StateObjectReference> GetUnitsOnTile(const out TTile kTile);
native function bool AreOtherUnitsOnTile( const out TTile kTile, XComGameState_Unit UnitStateObject );
native function array<Actor> GetActorsOnTile( const out TTile kTile, optional bool bIgnoreUnits = false );
...
native function bool GetAllActorsTrace(const out vector StartLocation, const out vector EndLocation, out array<ActorTraceHitInfo> HitInfo, optional Vector Extents );
//============================================================================
//
...
native function EventListenerReturn OnGameplayTileEffectUpdate(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData);
...
static function EventListenerReturn OnObjectMoved(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{	
...
function bool TeamInRadiusOfUnit( ETeam team, int tile_radius, XGUnit Unit)
{
	...
	foreach OutTiles( TilePair )
	{
		...
		UnitReferences = GetUnitsOnTile( TilePair.Tile );
		foreach UnitReferences(UnitReference)
		{
...
static function EventListenerReturn OnEnvironmentalDamageOccurred(Object EventData, Object EventSource, XComGameState GameState, Name Event, Object CallbackData)
{	
	...
	if(EnvironmentDamage != none && (`CHEATMGR == None || !`CHEATMGR.bDisableSecondaryFires))
	{
		   ...
		   EnvironmentDamage.bAffectFragileOnly != true) 
		{
			...
			*/
			...
			for(Index = CandidateFireTiles.Length - 1; Index >= 0; --Index)
			{
				if (class'X2Effect_ApplyFireToWorld'.static.TileEffectsPreventFire( CandidateFireTiles[Index].Tile, WorldData ))
					CandidateFireTiles.Remove( Index, 1 );
			}
...
native function CollectConeTiles(out array<TTile> Tiles, int SourceUnitID, int TargetUnitID, float ConeLength, float ConeEndDiameter);
