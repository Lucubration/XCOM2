...
var bool bAllowCacheRecycling; //Prevent cache recycling during the loading flurry for creation and removal. Avoiding the mc cache desync in flash. HACK! bsteiner 7/19/2017 
...
simulated function OnInit()
{
	...
	Pres.SetTimer(3.0,false, 'AllowCacheRecycling', self);
}

function RefreshCacheRecyclingTimer()
{
	if( !bAllowCacheRecycling && Pres.IsTimerActive('AllowCacheRecycling') )
	{
		`log("RESET cache recycle timer", , 'uicore');
		Pres.ClearTimer('AllowCacheRecycling');
		Pres.SetTimer(3.0, false, 'AllowCacheRecycling', self);
	}
}

function AllowCacheRecycling()
{
	`log("ACTIVATED AllowCacheRecycling Movie= " $ Name, , 'uicore');
...
defaultproperties
{
	...
	bAllowCacheRecycling = true;  //TEMP bsteiner 
}
