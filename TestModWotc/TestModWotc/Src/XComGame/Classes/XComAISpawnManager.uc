...
var private bool bDoingPrePlacedSpawning;
...
struct native CombinedSpawnRestrictions
{
	...
	structcpptext
	{
		...
		void AddLeaderSpawnDistributionInfo(const FSpawnDistributionList& SpawnDistribution, INT ForceLevel);
		void AddFollowerSpawnDistributionInfo(const FSpawnDistributionList& SpawnDistribution, INT ForceLevel);

		void Clear()
		{
			LeaderInclusionExclusionList.Clear();
			FollowerInclusionExclusionList.Clear();
			TheLostRestricted = CiviliansRestricted = AdventRestricted = AliensRestricted = FALSE;
		}

	private:
		void AddSpawnDistributionInfo(TInclusionExclusionList<FName>& List, const FSpawnDistributionList& SpawnDistribution, INT ForceLevel);
	}
};
...
struct native PodSpawnInfo
{
	...
	// PrePlacedEncounters can specify a tag to identify a particular spawn group
	var Name PrePlacedEncounterTag;

	// If specified, this group should consist of the indicated reward unit passed from strategy
	var Name StrategyUnitTag;
	...
	// If specified, this group will attempt to patrol this zone instead of one constructed with the EncounterZone settings.
	var Name EncounterZoneVolumeTag;
	...
	// Disable this group from being able to scamper
	var bool bDisableScamper;

	// Don't spawn this group as part of pre-mission spawning.
...
// standard reinforcements explicitly spawn at oblique angles to the LOP/recent reinforcement groups
var config float ReinforcementSpawningAngularWeight;
var config float ReinforcementSpawningDistanceWeight;
var config float ReinforcementSpawningSizeWeight;

// LOP reinforcements always try to respect the LOP
var config float LOPSpawningAngularWeight;
var config float LOPSpawningDistanceWeight;
var config float LOPSpawningSizeWeight;
...
function native SelectSpawnGroup(
	...
	const out MissionSchedule ActiveMissionSchedule,
	const out array<name> ActiveSitReps,
...
function native Vector SelectReinforcementsLocation(
	const XComGameState_AIReinforcementSpawner InstigatingSpawner,
	const out Vector TargetLocation,
	int IdealSpawnTilesOffset,
	bool bSpawnInXComLOS,
	bool bDontSpawnInXComLOS,
	bool bRequiresVerticalClearance,
	bool bAlwaysOrientAlongLOP);

function native SelectPodAtLocation(out PodSpawnInfo SpawnInfo, int InForceLevel, int InAlertLevel, const out array<name> ActiveSitReps);

function native XGAIGroup SpawnPodAtLocation(
	ref XComGameState NewGameState, 
	out PodSpawninfo SpawnInfo, 
	bool bSpawnInXComLOS,
	bool bDontSpawnInXComLOS,
	bool bDontSpawnInHazards);
...
function StateObjectReference CreateUnit( 
	Vector Position, 
	Name CharacterTemplateName, 
	ETeam Team, 
	bool bAddToStartState, 
	optional bool RollForLoot, 
	optional XComGameState NewGameState, 
	optional const XComGameState_Unit ReanimatedFromUnit = None, 
	optional string CharacerPoolName, 
	optional bool bDisableScamper, 
	optional bool bCopyReanimatedFromUnit,
	optional int AddToAIGroupID = -1,
	optional bool bCopyReanimatedStatsFromUnit)
{
...
private event XComGameState_Unit CreateUnitInternal( 
	...
	bool bRollForLoot, 
	bool PerformAIUpdate, 
	XComGameState_Unit ReanimatedFromUnit, 
	optional string CharacerPoolName, 
	optional bool bDisableScamper, 
	optional bool bCopyReanimatedFromUnit,
	optional int AddToAIGroupID = -1,
	optional bool bCopyReanimatedStatsFromUnit)
{
	...
	UnitState.PostCreateInit(NewGameState, CharacterTemplate, PlayerState.ObjectID, UnitTile, Team != eTeam_XCom, bUsingStartState, PerformAIUpdate, ReanimatedFromUnit, CharacerPoolName, bDisableScamper, bCopyReanimatedFromUnit, bCopyReanimatedStatsFromUnit);
	...
	if (!CharacterTemplate.bIsCosmetic)
	{
		if( AddToAIGroupID > 0 )
		{
			NewGroupState = XComGameState_AIGroup(NewGameState.ModifyStateObject(class'XComGameState_AIGroup', AddToAIGroupID));
		}
		else
		{
...
private event UpdateAIPlayerData(XComGameState NewGameState)
{
	...
	if( kAIPlayer != None )
	{
		...
		if( kPlayerData != None )
		{
			kPlayerData = XComGameState_AIPlayerData(NewGameState.ModifyStateObject(class'XComGameState_AIPlayerData', iAIDataID));
			kPlayerData.UpdateGroupData(NewGameState);
		}
	}
}

native function CacheFireTiles();

// Calculate the average location amongs all living XCom team members
native function Vector GetCurrentXComLocation();

native function int DEBUGFindTileSupportedSize(int InX, int InY, int InZ, int InHeight = 2, int IdealSize = 4) const;
...
simulated function DrawDebugLabel(Canvas InCanvas)
{
	...
	local array<name> EncounterZoneVolumeTags;
	...
	SightRadius = class'X2CharacterTemplateManager'.static.GetCharacterTemplateManager().FindCharacterTemplate('Soldier').GetCharacterBaseStat(eStat_SightRadius);
	...
	foreach History.IterateByClassType(class'XComGameState_AIGroup', AIGroup)
	{
		...
		if( CurrentEncounterZoneIndex == INDEX_NONE ||
			...
			EncounterZoneVolumeTags[CurrentEncounterZoneIndex] != AIGroup.MyEncounterZoneVolumeTag)
		{
			...
			EncounterZoneVolumeTags.AddItem(AIGroup.MyEncounterZoneVolumeTag);
		}
	...
	DebugString = "Mission = "$ ActiveMissionSchedule.ScheduleID
		...
		$ ", Advent = " $ AdventCount
		$ ", Total = " $ AICount
		$ ", Difficulty = " $ `TACTICALDIFFICULTYSETTING;
	...
	for( CurrentEncounterZoneIndex = 0; CurrentEncounterZoneIndex < EncounterZoneOffsetAlongLOPs.Length; ++CurrentEncounterZoneIndex )
	{
		DebugEncounterZone = BuildEncounterZone(
			...
			EncounterZoneVolumeTags[CurrentEncounterZoneIndex]);
...
function ClearCachedFireTiles()
{
	CachedFireTiles.Length = 0;
	bCachedFire = false;
}
...
private:
	...
	void SelectAndSpawnTurrets(const TArray<FTTile>& XComOccupiedTiles, const TArray<FName> &ActiveSitReps);

	// Select and Spawn a set of civilians on the map.
	void SelectAndSpawnCivilians(const TArray<FTTile>& XComOccupiedTiles, const TArray<FName>& ActiveSitReps);
	...
	void SelectPodMembers(
		...
		const FCombinedSpawnRestrictions& SpawnRestrictions, 
		const FSpawnDistributionList* LeaderSpawnDistributionList, 
		const FSpawnDistributionList* FollowerSpawnDistributionList, 
		TArray<UX2CharacterTemplate*>& SelectedCharacterTemplates,
		INT LeaderForceLevelMod,
		const TArray<FName> &ActiveSitReps);
	...
	// Spawn a unit passed in from the strategy game which matches a specific tag
	UXComGameState_Unit* SpawnUnitFromStrategyWithTag(
		FName StrategyUnitTag, 
		AXGAIGroup& AIGroup, 
		const TArray<FTTile>* RequiredLOSTiles,
		const TArray<FTTile>* RequiredNoLOSTiles, 
		UBOOL bAllowSpawnInHazards);
	...
	const UX2CharacterTemplate* SelectPodLeader( 
		...
		const FCombinedSpawnRestrictions& SpawnRestrictions,
		const FSpawnDistributionList* SpawnDistributionList, 
	...
	UXComGameState_Unit* SpawnCharacterIntoPod(
		...
		class UXComGameState_AIGroup* GroupState,
		const TArray<FTTile>* RequiredLOSTiles,
		const TArray<FTTile>* RequiredNoLOSTiles,
		UBOOL bAllowSpawnInHazards);
	...
	UBOOL FindClosestSpawnLocationWithLOS(
		const UX2CharacterTemplate& CharacterTemplate,
	...
	// Searches for a spawn location which meets the LOS requirements that best matches the specified Ideal Size, Distance, and Angular direction
	// WRT the specified source tile.
	// Returns the SourceTile if no other valid tiles exist on the map.
	FTTile FindIdealSpawnLocationWithLOS(
		const FTTile& SourceTile,

		// angular weighting info
		const FTTile& DirectionTargetTile,
		FLOAT AngularWeightFactor,

		// distance weighting info
		INT IdealDistance,
		FLOAT DistanceWeightFactor,

		// size weighting info
		INT IdealSize,
		FLOAT SizeWeightFactor,
		...
		const TArray<FTTile>* RequiredNoLOSTiles,
		FLOAT MaxSightRangeMeters,
		UBOOL bRequiresVerticalClearance) const;


	// Helper to find the supported size of the TargetTile (up to the specified IdealSize)
	INT FindTileSupportedSize(
		class UXComWorldData& WorldData,
		const FTTile& TargetTile,
		INT IdealSize,
		INT IdealHeight,
		UBOOL bAllowSpawnInFire,
		UBOOL bAllowSpawnInPoison) const;


	// Helper to determine if LOS requirements are met WRT the specified TargetTile
	UBOOL MeetsLOSRequirements(
		class UXComWorldData& WorldData,
		const FTTile& TargetTile,
		const UX2CharacterTemplate& CharacterTemplate,
		const TArray<FTTile>* RequiredLOSTiles,
