...
simulated function StartMatch()
{
	...
	class'X2TacticalGameRuleset'.static.ReleaseScriptLog( "XComTacticalGRI::StartMatch: Dropship="$bDropshipLaunch$", AutoTests="$bAutoTests$", bTacticalQuickLaunch="$bTacticalQuickLaunch );
...
simulated function BuildStartStateForPIE()
{
	...
	if( Profile != none )
	{
		...
		foreach TacticalStartState.IterateByClassType(class'XComGameState_Unit', ExamineUnit)
		{
			...
			{
				HeadquartersStateObject.Squad.AddItem(ExamineUnit.GetReference());
			}
		}
	}	
	else
	...
	{
		HeadquartersStateObject = XComGameState_HeadquartersXCom( TacticalStartState.CreateNewStateObject( class'XComGameState_HeadquartersXCom' ) );
...
{   	
	...
	if (m_kBattle.GetDesc().m_arrSecondWave.Length > eGO_RandomSeed)
	{
		SecondWaveRandomSeed = m_kBattle.GetDesc().m_arrSecondWave[eGO_RandomSeed];
	}
	
	if( (!m_kBattle.GetDesc().bUseFirstStartTurnSeed) && (!bStandardLoad || SecondWaveRandomSeed > 0) )
	{	
	...
	{
		class'X2TacticalGameRuleset'.static.ReleaseScriptLog( "TacticalGRI::InitBattle: World="$WorldData$", Size="$WorldData.NumX );
