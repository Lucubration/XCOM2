...
/** If TRUE, movie will adjust it's viewport to fit the game screen size. If not, UI movie size is independent of screen size. */
var bool		bAdjustToFullRenderViewport;
...
struct native ASValue
{
  ...
  var() init string s;
	...
	structcpptext
	{
		...
		FORCEINLINE FASValue* operator=(FASValue* value)
		{
			if (value->Type < AS_Max)
			{
				*this = *value;
			}
			else
 ...
 */
// FIRAXIS BEGIN
// XComChange: Making these public so they can be accessed by the UI classes through the UIMovie_2D - sbatista060211
public native noexport final function ActionScriptVoid(string path);
public native noexport final function int ActionScriptInt(string path);
public native noexport final function float ActionScriptFloat(string path);
public native noexport final function string ActionScriptString(string path);
public native noexport final function GFxObject ActionScriptObject(string path);
//FIRAXIS END
...
defaultproperties
{
	...
	bAdjustToFullRenderViewport=TRUE
