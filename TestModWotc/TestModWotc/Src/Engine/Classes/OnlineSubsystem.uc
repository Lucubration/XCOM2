...
/** Result of attempting to access player storage */
enum EStorageResult
{
	/** Player storage exists and was successfully read */
	eSR_Success,
	/** Player storage exists but was corrupt */
	eSR_Corrupt,
	/** Player storage was not found (file does not exist) */
	eSR_NotFound,
};
...
struct native OnlineContent
{
	...
	var const native transient Map_Mirror EDATFileContents{TMap<FString,FString>};
};
...
//Contains information about the saved game - anything the UI would want to know about
struct native SaveGameHeader
{
	var int VersionNumber;		//
	var int HeaderSize;			//Size in bytes of the header
	var int HeaderCRC;			//CRC of the header for corruption detection
	var int UncompressedSize;
	var int GameNum;
	var int SaveID;         // unique "slot" identifier for this save file
	var int SaveDataCRC;	
	var string Description;
	var string Time;
	var string MapCommand; //The engine command to use when launching this save
	var string DLCPacks;   //Deprecated
	var string Language;	
	var bool bIsTacticalSave;
	var bool bIsIronman;
	var bool bIsAutosave;
	var bool bIsQuicksave;
	var int ArchiveFileVersion;         // Values used for the Archiver when serializing objects
	var int ArchiveFileLicenseeVersion; // **	
	var string CampaignStartTime;		// Time that the campaign was started
	var string MapImage;				// Used in the saved game list UI
	var string PlayerSaveDesc;			// Players can enter descriptions for their save with the saved game UI, which this field stores
	var array<string> DLCPackNames;
	var array<string> DLCPackFriendlyNames; //Since DLC can be removed making the friendly name inaccessible, store it here
	var int Mission;
	var int Month;
	var int Turn;
	var int Action;
...
struct native OnlineSaveGameDataMapping
{
	...
	/** Meta information about the save */
...
struct native OnlineSaveGame extends OnlineContent
{
	...
	var init array<OnlineSaveGameDataMapping> SaveGames;
...
// FTickableObject interface
	...
	static FString UniqueNetIdToHexString( const FUniqueNetId& IdToConvert );
	 ...
	 *
	 * @param bSessionEnded		Wether or not the game session has ended
	 */
...
static final native noexportheader function string UniqueNetIdToHexString(const out UniqueNetId IdToConvert);
...
static function DumpGameSettings(const OnlineGameSettings GameSettings)
{
	...
	`Log("      BuildUniqueId: "$GameSettings.BuildUniqueId);
	// FIRAXIS end -tsmith
...
event bool GetUniquePlayerNetId(int PlayerNumber, out UniqueNetId PlayerId)
{
	return PlayerInterface.GetUniquePlayerId(PlayerNumber, PlayerId);
}

event string GetPlayerNetNickname(byte LocalUserNum)
{
	return PlayerInterface.GetPlayerNickname(LocalUserNum);
}
