...
/**
 * Delegate fired when the XMB UI is closed
 * @success FALSE if the UI is canceled
 */
delegate OnLoginUIComplete( bool success );

/**
 * Adds the delegate to the list to be notified when the UI is closed.
 *
 * @param LoginUICompleteDelegate the delegate to add
 */
...
/**
 * Notifies the online subsystem that we are resetting all login information and
 * any cached login information must be discarded
 */
function ResetLogin();
