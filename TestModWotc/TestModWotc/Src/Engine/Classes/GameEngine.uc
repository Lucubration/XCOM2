...
var init array<FullyLoadedPackagesInfo> PackagesToFullyLoad;
...
/* Object used for determing appropriate graphics setting. */
var GPUAutoDetect AutoDetect;
...
/** 
 *  Returns if Steamworks is initialized. 
 *  hack to see if Steamworks is initialized. needed because you can have an OnlineSubsystemSteamworks without initializing steam
 *  (via config var or commandline option). in that case all functions just pass through the default implementation. -tsmith 
 */
native static final function bool IsSteamworksInitialized();
// FIRAXIS end -tsmith

/* Initiates a Graphics Auto Detect, creating the object if needed.*/
delegate OnFinish();
final function RunGPUAutoDetect(bool bForceCheck, delegate<OnFinish> FinishDelegate)
{
	if (AutoDetect == none)
		AutoDetect = new class'GPUAutoDetect';

	AutoDetect.StartCheck(bForceCheck, FinishDelegate);
}

/* Forcefully end the AutoDetect */
final function ForceEndGPUAutoDetect()
{
	if (AutoDetect != none)
	{
		AutoDetect.ForceEndCheck();
	}
}

final function bool IsGPUAutoDetectRunning()
{
	return AutoDetect != none && AutoDetect.m_bIsRunning;	
}
...
cpptext
{
	...
	// Firaxis BEGIN
	// RAM - adding the ability to export a level in text form, useful for checking game state invariance ( eg. for save / load )
	void ExportMap(const TCHAR* InFilename);

	// jboswell: hook for the engine to load dynamic (not map referenced) content
	...
	virtual void FillRequiredContentInfo_Perks(class FRequiredContentInfo_Perks& kRequiredContentInfo_Perks) {}   
	...
	virtual void SetRequiredContentFromRequiredContentInfo_Perks(const class FRequiredContentInfo_Perks& kRequiredContentInfo_Perks) {}
	// Firaxis END
	...
	/**
	 * Firaxis begin - dburchanowski
	 * Plotswap data lives at the xcomgame level, but the blueprints were put in at the engine level.
	 * For serious performance reasons we need to be able to swap the blueprint's mapname before it actually
	 * streams it, but this requires this hook down into the engine
	 */
...
defaultproperties
{
	OnlineEventMgrClassName="Engine.OnlineEventMgr"
}