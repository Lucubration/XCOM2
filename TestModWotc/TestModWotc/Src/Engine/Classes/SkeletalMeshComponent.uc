...
/** The static mesh that is used for static lighting */
var()   editoronly StaticMesh   LightingStaticMesh; // FIRAXIS ADDITION
...
var const native transient pointer FxCallbackStruct{ FaceFxCallbackStruct };

// FIRAXIS BEGIN
// Sockets from the SkeletalMesh and attached SkeletalMeshes
var	transient array<SkeletalMeshSocket>	Sockets;
var duplicatetransient bool							AddedDefaultSockets;
// FIRAXIS END
...
var transient native AnimNodeBlendList FaceGenNode; // This is for the facefx blend with gender blender.
// FIRAXIS END
...
/** Array to hold morph keys that do not match anything this component has - this indicates that they may relate to a child component. */
var	transient array<CurveKey>		MorphCurveKeys;
...
struct Attachment
{
	...
	var() name						BoneName;
	var   name						SocketName;		// BSG - track the socket name for the attachment for proper equip/unequip
...
// BEGIN FIRAXIS
/** If false, SetAnimPosition will not execute when our Owner has not been rendered recently. */
var() bool bAllowSetAnimPositionWhenNotRendered;
// END FIRAXIS
...
/** needed for cloth sim'ed SkelMeshComponent's ParentAnimComponent to get root motion updates before physics sim for cloth occurs*/
var bool bForceNoThreadedUpdate;
...
var () transient const bool bProcedurallyAnimated;
// FIRAXIS END

var(Rendering) bool bEditorAllowPlayLimits;
var transient native Array<int> FxBoneIndexTranslation;
...
native final function AttachComponent(ActorComponent Component,name BoneName,optional vector RelativeLocation,optional rotator RelativeRotation,optional vector RelativeScale,optional name SocketName);	// BSG - track the socket name for the attachment for proper equip/unequip
native final function DetachComponent(ActorComponent Component);


/**
 * Attach an ActorComponent to a Socket. AttachedSocket allows a socket on the attaching component to be specified where Socketname and SocketNameOnAttachment will be lined up.
 */

native final function AttachComponentToSocket(ActorComponent Component, name SocketName, optional name SocketNameOnAttachment);

// FIRAXIS BEGIN EPACE
// If bReplaceDuplicates then a socket with the same name in aSockets replaces the corresponding socket(s)
// with the same name.  Otherwise, this naively appends the sockets, but duplicates will be available.
// So if in doubt, set bReplaceDuplicates to true.
 ...
 */
native final function bool GetSocketWorldLocationAndRotation(name InSocketName, out vector OutLocation, optional out rotator OutRotation, optional int Space, optional bool bSpam ); // FIRAXIS ADDITION : Space : 0 == World, 1 == Local (Component)

// Firaxis BEGIN
// RAM - adding to support attaching effects to characters based on damage, but this is generally useful to have
/**
 *  Retrieve the socket on the character nearest to the specified location, returns None if no valid sockets fufill the arguments
 *  @param CheckLocation a world space location to check for sockets
 *  @param CheckDistance an optional world space distance that allows results to be filtered
 */
native final function SkeletalMeshSocket GetNearestSocket(vector CheckLocation, optional float CheckDistance = 0.0f);
// Firaxis END
...
/** Same as AttachedComponents above, but takes a socket or bone name (use one or the other). */
native final iterator function AttachedComponentsOnBone(class<ActorComponent> BaseClass, out ActorComponent OutComponent, optional name InSocketName, optional name InBoneName);
...
// FIRAXIS BEGIN - Ryan Baker
/** Unmounts the facefxanimset from the actors */
native final function UnMountCinematicFaceFX();
// FIRAXIS END - Ryan Baker
...
//	it's been removed from the latest FaceFx, and doesn't appear to have any direct analog in the new system
//	if it turns out to be functionality that was used, we'll figure something out - --Ned
...
event bool PlayParticleEffect( const AnimNotify_PlayParticleEffect AnimNotifyData )
{
	...
	if (AnimNotifyData.bAttach)
	{
		...
		{
			foreach AttachedComponentsOnBone( class'ParticleSystemComponent', PSC, AnimNotifyData.SocketName, AnimNotifyData.BoneName )
			{
		...
		AttachBoneIndex = MatchRefBone(AnimNotifyData.BoneName);
		AttachSocket = GetSocketByName(AnimNotifyData.SocketName);
	...
	{
		// find the location
		if( SkeletalMesh == none) // FIRAXIS ADDITION - Play the effect at a reasonable place (and not crash) if the component has no mesh
		{
			Loc = Owner.Location;
			Rot = rot( 0, 0, 1 );
		}
		else if( AnimNotifyData.SocketName != '' )
		{
	...
	if (PSC != none && AnimNotifyData.bRemoveOnPawnDeath)
	{
		...
		while (aOwner != none)
		{
			if (aOwner.NotifyPSCWantsRemoveOnDeath(PSC))
				break;
...
// FIRAXIS begin
event StopParticleEffect( const AnimNotify_StopParticleEffect AnimNotifyData )
{
	local ParticleSystemComponent PSC;
	...
	foreach AttachedComponentsOnBone( class'ParticleSystemComponent', PSC, AnimNotifyData.SocketName, AnimNotifyData.BoneName )
	{
		if (PSC.Template != AnimNotifyData.PSTemplate)
			continue;

		if (AnimNotifyData.bImmediate)
		{
			PSC.SetActive(false);
			PSC.ResetToDefaults();
		}
		else PSC.SetActive(false);
	}
}
// FIRAXIS end
...
defaultproperties
{
	...
	bAllowSetAnimPositionWhenNotRendered=true // FIRAXIS CHANGE
	...
	ApexClothingRBCollideWithChannels=(Default=FALSE,BlockingVolume=TRUE,GameplayPhysics=TRUE,EffectPhysics=TRUE,ClothingCollision=TRUE)
	//bAutoFreezeApexClothingWhenNotRendered is OFF because otherwise the cloth may penetrate something animating into it while it is frozen. 
	bAutoFreezeApexClothingWhenNotRendered=FALSE 
...
// FIRAXIS BEGIN jboswell
	bProcedurallyAnimated = false
	CanBlockCamera = true // FIRAXIS jboswell
	AddedDefaultSockets = FALSE
// FIRAXIS END

	bCastDynamicShadow=TRUE
	bEditorAllowPlayLimits=FALSE
	//debug
	//bDisplayBones=1
}
