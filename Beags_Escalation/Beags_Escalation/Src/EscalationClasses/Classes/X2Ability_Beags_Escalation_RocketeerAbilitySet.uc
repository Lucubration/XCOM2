...
var config array<ArmorPierceValue> HEATWarheadsArmorPierce;
...
static function X2DataTemplate HEATWarheads()
{
	...
	for (i = 0; i < default.HEATWarheadsArmorPierce.Length; i++)
		`LOG("Beags Escalation: HEAT Warheads template name " @ string(default.HEATWarheadsArmorPierce[i].TemplateName) @ " armor pierce=" @ string(default.HEATWarheadsArmorPierce[i].ArmorPierce));
...
static function X2DataTemplate FireInTheHole()
{
	...
	NoMoveCondition = new class'X2Condition_UnitValue';
	NoMoveCondition.AddCheckValue('MovesThisTurn', 0, eCheck_Exact);
	ToHitEffect.ToHitConditions.AddItem(NoMoveCondition);
