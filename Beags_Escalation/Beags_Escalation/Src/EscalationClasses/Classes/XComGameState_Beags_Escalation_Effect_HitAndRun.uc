		...
		AbilityContext.InputContext.SourceObject.ObjectID != AbilityContext.InputContext.PrimaryTarget.ObjectID)
	{
		...
		if (SourceUnitState != none && !SourceUnitState.GetUnitValue(default.HitAndRunUsedName, UnitValue))
		{
			if (`TACTICALRULES.GetCachedUnitActionPlayerRef().ObjectID == SourceUnitState.ControllingPlayer.ObjectID)
			{
				...
				PrimaryTargetUnitState = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(AbilityContext.InputContext.PrimaryTarget.ObjectID,, AbilityContext.AssociatedState.HistoryIndex - 1));
				if (PrimaryTargetUnitState != none)
				{
					...
					{
						`TACTICALRULES.VisibilityMgr.GetVisibilityInfo(SourceUnitState.ObjectID, PrimaryTargetUnitState.ObjectID, VisibilityInfoFromSource, AbilityContext.AssociatedState.HistoryIndex - 1);
