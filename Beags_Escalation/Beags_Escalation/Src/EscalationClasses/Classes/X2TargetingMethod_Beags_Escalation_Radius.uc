class X2TargetingMethod_Beags_Escalation_Radius extends X2TargetingMethod;
...
function Init(AvailableAction InAction)
{
	...
	// Partial squadsight may extend the range of radius targeted abilities (e.g. DZ + Suppression or Flush)
	if (UnitState.HasSquadsight())
	{
		...
		// If a squadsight range was defined, add it to the targeting range
		if (SquadsightRange > 0)
			TargetingRange += `UNITSTOMETERS(SquadsightRange);
	}
...
{
	return Cursor.GetCursorFeetLocation();
}
