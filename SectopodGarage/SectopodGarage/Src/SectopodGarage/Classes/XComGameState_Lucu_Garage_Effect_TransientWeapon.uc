...
class XComGameState_Lucu_Garage_Effect_TransientWeapon extends XComGameState_BaseObject;
...
function EventListenerReturn OnTacticalGameEnd(Object EventData, Object EventSource, XComGameState GameState, Name EventID)
{
		...
		if (ItemState != none)
		{
			...
			if (UnitState != none)
			{
				...
				if (AbilityState != none)
				{
					...
					`LOG("Lucu Garage: Transient Item " @ ItemState.GetMyTemplateName() @ " removed as source weapon for unit " @ UnitState.GetFullName() @ " ability " @ AbilityState.GetMyTemplateName() @ ".");
				}
				else
				{
					`LOG("Lucu Garage: Transient Item " @ ItemState.GetMyTemplateName() @ " not removed as source weapon for unit " @ UnitState.GetFullName() @ " ability " @ AbilityState.GetMyTemplateName() @ " (ability state not found).");
				}
				...
				if (AbilityRef.ObjectID > 0)
				{
					...
					if (AbilityState != none)
					{
						...
						`LOG("Lucu Garage: Transient Item " @ ItemState.GetMyTemplateName() @ " ability " @ AbilityState.GetMyTemplateName() @ " state removed from history.");
					}
					else
					{
						`LOG("Lucu Garage: Transient Item " @ ItemState.GetMyTemplateName() @ " ability " @ AbilityState.GetMyTemplateName() @ " state not removed from history (item state not found).");
					}
				}

				`LOG("Lucu Garage: Transient Item " @ ItemState.GetMyTemplateName() @ " removed from unit " @ UnitState.GetFullName() @ " inventory.");
			}
			else
			{
				`LOG("Lucu Garage: Transient Item " @ ItemState.GetMyTemplateName() @ " not removed from unit inventory (unit state not found).");
			}
	
			// Remove the transient item's gamestate object from history
			NewGameState.RemoveStateObject(ItemRef.ObjectID);

			`LOG("Lucu Garage: Transient " @ ItemState.GetMyTemplateName() @ " state removed from history.");
		}
		else
		{
			`LOG("Lucu Garage: Transient Item not removed from unit inventory (item state not found).");
		}
	}
	...
	`LOG("Lucu Garage: Transient Item passive effect unregistered from events.");
