class X2Effect_Lucu_Garage_TransientWeapon extends X2Effect_Persistent;
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	local XComGameState_Lucu_Garage_Effect_TransientWeapon		TransientItemEffectState;
		...
		if (UnitState != none)
		{
			...
			if (TransientItemTemplate != none && TransientItemTemplate.InventorySlot == eInvSlot_Utility)
			{
				...
				if (TransientItemState == none)
				{
					...
					`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " state created.");
					...
					if (ItemSetAbilityState != none)
					{
						...
						if (SourceAmmoRef.ObjectID > 0)
							`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " set as source ammo for unit " @ UnitState.GetFullName() @ " ability " @ AbilityTemplateName @ ".");
						else
							`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " set as source weapon for unit " @ UnitState.GetFullName() @ " ability " @ AbilityTemplateName @ ".");
					}
					else
					{
						...
						if (AbilityTemplate != none)
						{
							...
							`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " ability state " @ AbilityTemplateName @ " created and added to unit " @ UnitState.GetFullName() @ ".");
						}
						else
						{
							`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " ability state " @ AbilityTemplateName @ " not created for unit " @ UnitState.GetFullName() @ " (ability template not found).");
						}
					}

					`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " added to unit " @ UnitState.GetFullName() @ " inventory.");
				...
				else
				{
					...
					if (TransientItemTemplate != none && TransientItemTemplate.bMergeAmmo)
					{
						...
						`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " merged " @ string(TransientItemTemplate.iClipSize) @ " ammo into unit " @ UnitState.GetFullName() @ " inventory.");
					}
					else
						`LOG("Lucu Garage: Transient Item " @ TransientItemState.GetMyTemplateName() @ " not added to unit " @ UnitState.GetFullName() @ " inventory (duplicate item).");
				}

				// Create effect component and attach it to GameState_Effect, adding the new state object to the NewGameState container
				TransientItemEffectState = XComGameState_Lucu_Garage_Effect_TransientWeapon(NewGameState.CreateStateObject(class'XComGameState_Lucu_Garage_Effect_TransientWeapon'));
				if (CreatedNewItem)
				{
				...
				`LOG("Lucu Garage: Transient Item passive effect registered for events.");
			}
			else
			{
				`LOG("Lucu Garage: Transient Item effect inventory item not added (utility weapon template not found).");
			}
		}
		else
		{
			`LOG("Lucu Garage: Transient Item effect inventory item not added (unit state not found).");
		}
	}
...
static function XComGameState_Lucu_Garage_Effect_TransientWeapon GetEffectComponent(XComGameState_Effect Effect)
{
    if (Effect != none) 
        return XComGameState_Lucu_Garage_Effect_TransientWeapon(Effect.FindComponentObject(class'XComGameState_Lucu_Garage_Effect_TransientWeapon'));
    return none;
}
