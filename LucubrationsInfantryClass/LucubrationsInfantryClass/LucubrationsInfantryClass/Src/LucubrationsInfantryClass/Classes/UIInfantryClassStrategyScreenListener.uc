class UIInfantryClassStrategyScreenListener extends UIStrategyScreenListener;
...
event OnInit(UIScreen Screen)
{
    if (IsInStrategy())
    {
		...
		//AddSoldierUnlockTemplate('OfficerTrainingSchool', 'SteadfastUnlock');
		AddSoldierUnlockTemplate('OfficerTrainingSchool', 'ExtraConditioningUnlock');
...
static function ApplyDeepReservesToSquadWoundTimers()
{
	...
	for (i = 0; i < XComHQ.Squad.Length; ++i)
	{
		if (XComHQ.Squad[i].ObjectID > 0)
		{
			...
			if (UnitState.HasSoldierAbility(class'X2Ability_InfantryAbilitySet'.default.DeepReservesAbilityName))
			{
				foreach History.IterateByClassType(class'XComGameState_HeadquartersProjectHealSoldier', HealProject)
				{
					...
					if (HealProject.ProjectFocus == UnitState.GetReference())
					{
						...
						HealProject.ModifyProjectPointsRemaining(class'X2Ability_InfantryAbilitySet'.default.DeepReservesWoundPercentToHeal);
