...
class X2Effect_EstablishedDefensesOverwatch extends X2Effect_Persistent;
...
function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	...
	EventMgr.RegisterForEvent(EffectObj, 'EstablishedDefensesTriggered', EffectGameState.TriggerAbilityFlyover, ELD_OnStateSubmitted, , UnitState);
}
...
function bool PostAbilityCostPaid(XComGameState_Effect EffectState, XComGameStateContext_Ability AbilityContext, XComGameState_Ability kAbility, XComGameState_Unit SourceUnit, XComGameState_Item AffectWeapon, XComGameState NewGameState, const array<name> PreCostActionPoints, const array<name> PreCostReservePoints)
{
	...
	local X2Effect_EstablishedDefensesArmorBonus ArmorEffect;
	...
	if (kAbility.GetMyTemplate().DataName == 'Overwatch' || kAbility.GetMyTemplate().DataName == 'PistolOverwatch' || kAbility.GetMyTemplate().DataName == 'SniperRifleOverwatch' || kAbility.GetMyTemplate().DataName == 'LongWatch')
	{
		...
		if (HistoricalUnit.ActionPoints.Length != PreCostActionPoints.Length)
		{
			...
			if (EstablishedDefensesOverwatchPoints > 0)
			{
				...
				SourceUnit.SetUnitFloatValue(class'X2Ability_InfantryAbilitySet'.default.EstablishedDefensesOverwatchPointsName, EstablishedDefensesOverwatchPoints, eCleanup_BeginTurn);
				...
				for (i = 0; i < EstablishedDefensesOverwatchPoints; i++)
				{
					...
					ApplyData.EffectRef.SourceTemplateName = class'X2Ability_InfantryAbilitySet'.default.EstablishedDefensesOverwatchArmorAbilityName;
					...
					ArmorEffect = X2Effect_EstablishedDefensesArmorBonus(class'X2Effect'.static.GetX2Effect(ApplyData.EffectRef));
	...
	else if (kAbility.GetMyTemplate().DataName == 'OverwatchShot' || kAbility.GetMyTemplate().DataName == 'PistolOverwatchShot')
	{
		...
		if (SourceUnit.ReserveActionPoints.Length != PreCostReservePoints.Length)
		{
			...
			if (SourceUnit.GetUnitValue(class'X2Ability_InfantryAbilitySet'.default.EstablishedDefensesOverwatchPointsName, UnitValue))
			{
				...
				if (EstablishedDefensesOverwatchPoints > 0)
				{
					...
					if (AbilityState != none)
					{
						...
						EventMgr = `XEVENTMGR;
						EventMgr.TriggerEvent('EstablishedDefensesTriggered', AbilityState, SourceUnit, NewGameState);
