class X2Effect_ShakeItOff extends X2Effect_PersistentStatChange;
...
function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	...
	EventMgr.RegisterForEvent(EffectObj, 'ShakeItOffTriggered', EffectGameState.TriggerAbilityFlyover, ELD_OnStateSubmitted, , UnitState);
}
		...
		foreach OldUnit.AffectedByEffects(OldEffectStateRef)
		{
				...
				OldEffectState.GetX2Effect().EffectName == class'X2Effect_MindControl'.default.EffectName)
			{
				if (OldEffectState.ApplyEffectParameters.TargetStateObjectRef.ObjectID == NewUnit.ObjectID)
				{
					...
					{
						Effect.CleansedVisualizationFn(VisualizeGameState, BuildTrack, 'AA_Success');
					}
					else
					{
						Effect.AddX2ActionsForVisualization_Removed(VisualizeGameState, BuildTrack, 'AA_Success', OldEffectState);
					}
				...
				{
					Effect = OldEffectState.GetX2Effect();
					Effect.AddX2ActionsForVisualization_RemovedSource(VisualizeGameState, BuildTrack, 'AA_Success', OldEffectState);
				}
			}
		}
