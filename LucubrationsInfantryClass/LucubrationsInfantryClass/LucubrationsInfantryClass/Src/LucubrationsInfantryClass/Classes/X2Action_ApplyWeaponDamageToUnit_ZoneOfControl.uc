// Literally a copy of X2Action_ApplyWeaponDamageToUnit except that I replace the counterattack animation
class X2Action_ApplyWeaponDamageToUnit_ZoneOfControl extends X2Action 
...
var float																	HitReactDelayTimeToDeath;
...
function Init(const out VisualizationTrack InTrack)
{
	...
	local X2Action_ApplyWeaponDamageToUnit_ZoneOfControl OtherAction;
	...
	//`LOG("Lucubration Infantry Class: Zone of Control X2Action_ApplyWeaponDamageToUnit_ZoneOfControl initializing.");
	...
	if (AbilityContext != none || TickContext != none)
	{
		...
		if (!bWasHit && ((OriginatingEffect.bApplyOnHit && !OriginatingEffect.bApplyOnMiss ) || m_iDamage + m_iMitigated == 0))
		{
			...
			{
				OtherAction = X2Action_ApplyWeaponDamageToUnit_ZoneOfControl(InTrack.TrackActions[ActionIndex]);
	...
	else if (AreaDamageContext != None)
	{
		...
		VisualizationMgr.TrackHasActionOfType( Track, class'X2Action_ApplyWeaponDamageToUnit_ZoneOfControl', OutFirstDamageAction );
...
Begin:	
	...
	//`LOG("Lucubration Infantry Class: Zone of Control X2Action_ApplyWeaponDamageToUnit_ZoneOfControl executing.");
	...
	if (!bHiddenAction)
	{
		...
		else
		{
			if (ShouldPlayAnimation())
			{
				//`LOG("Lucubration Infantry Class: Zone of Control should play custom apply weapon damage action.");

				if(bCounterAttackAnim)
				{
					//`LOG("Lucubration Infantry Class: Zone of Control playing custom apply weapon damage action.");

					AnimParams.AnimName = 'FF_Fire_Xcom';
				}
				else
				{
					Unit.ResetWeaponsToDefaultSockets();
